...
import { clearCampusdownloadsData, setEdit } from '../../redux/campusDownloads.slice';
import CampusKeyOfficialUpdateForm from './Form';

const CampusKeyOfficialEditModal = () => {
  const dispatch = useDispatch();
  const { edit, currentId } = useSelector(campusDownloadsState);

  // Only fetch when we have a valid ID and are in edit mode
  const { data: campusDownloadsData, isLoading } = useRetrieveCampusDownloadsQuery(currentId, { skip: !currentId || !edit });
  ...
  const handleClose = () => {
    ...
    dispatch(clearCampusdownloadsData());
  };
    ...
    <AppDialog
      ...
      content={
        isLoading ? (
          ...
          <CampusKeyOfficialUpdateForm campusDownloadsData={campusDownloadsData} onClose={handleClose} />
        )
      }
    />
  );
