...
import { Add, Edit, Delete, Tag, ColorLens } from '@mui/icons-material';
...
} from './redux/projects.api';
import { IProjectTag, IProjectTagCreatePayload } from './redux/types';
...
const ProjectTags: React.FC = () => {
  ...
  const { data: tagsData, isLoading: isLoadingTags, error: tagsError } = useGetProjectTagsQuery();
  ...
  const [deleteTag, { isLoading: isDeleting }] = useDeleteProjectTagMutation();

  const isLoading = isCreating || isUpdating;

  const formik = useFormik<IProjectTagCreatePayload>({
    initialValues: {
      name: '',
      description: '',
      color: '#2196f3'
  ...
  const handleOpenDialog = (tag?: IProjectTag) => {
    if (tag) {
      setEditingTag(tag);
      formik.setValues({
        name: tag.name,
  ...
  if (tagsError) {
    return (
      <Alert severity="error" sx={{ m: 2 }}>
        Failed to load project tags. Please try again.
      </Alert>
    );
  }
  ...
  return (
    <Box>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
        <Typography variant="h4">Project Tags</Typography>
        ...
        <Card>
          <CardContent sx={{ textAlign: 'center', py: 8 }}>
            ...
            <Typography variant="body2" color="text.secondary" sx={{ mb: 3 }}>
              Create your first project tag to organize and categorize projects.
            </Typography>
            <Button variant="contained" startIcon={<Add />} onClick={() => handleOpenDialog()}>
              Create First Tag
            </Button>
