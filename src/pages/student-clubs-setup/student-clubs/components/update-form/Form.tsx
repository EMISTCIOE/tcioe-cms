import { Button, Grid } from '@mui/material';

// UI Components
import FormSection from '@/components/app-form/FormSection';
import MainCard from '@/components/cards/MainCard';

import useUpdateStudentClubs from '../../hooks/useUpdateStudentClubs';
import { IStudentClubsDetails } from '../../redux/types';
import { TStudentClubsUpdateFormDataType } from './config';

export interface IStudentClubsUpdateFormProps {
  studentClubsData?: IStudentClubsDetails;
  onClose?: () => void;
}

export default function StudentClubsUpdateForm({ studentClubsData, onClose }: IStudentClubsUpdateFormProps) {
  const { control, errors, watch, formFields, handleSubmit } = useUpdateStudentClubs({ studentClubsData, onClose });
  const formValues = watch();

  return (
    <form onSubmit={handleSubmit()}>
      <Grid container spacing={3} sx={{ pt: 1 }}>
        <Grid item xs={12}>
          <MainCard divider title={'Update Student Club'}>
            <FormSection<TStudentClubsUpdateFormDataType> fields={formFields} control={control} errors={errors} formValues={formValues} />
          </MainCard>
        </Grid>

        <Grid item xs={12} sx={{ display: 'flex', justifyContent: 'flex-end', gap: 2, flexWrap: 'wrap' }}>
          <Button variant="outlined" color="error" onClick={onClose}>
            Cancel
          </Button>
          <Button variant="contained" type="submit">
            Update
          </Button>
        </Grid>
      </Grid>
    </form>
  );
}
