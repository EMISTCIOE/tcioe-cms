...
// Component
const DetailView: React.FC<IDetailViewProps> = ({ studentClubsData, onClose }) => {
    ...
    return (
      <Paper elevation={3} sx={{ p: 4, textAlign: 'center' }}>
        <Typography variant="h5" mb={3}>
          Student Clubs Details Not Found
        </Typography>
        <CircularProgress />
      </Paper>
    );
    ...
    <MainCard sx={{ p: 0, overflow: 'hidden', position: 'relative' }}>
      ...
      >
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
          <Typography variant="h4">{studentClubsData?.name || 'Unknown StudentClubs'}</Typography>
          <Box sx={{ mt: 1 }}>
            <Chip
              size="small"
              variant="outlined"
              color={studentClubsData.isActive ? 'success' : 'error'}
              label={studentClubsData.isActive ? 'Active' : 'Inactive'}
              icon={studentClubsData.isActive ? <CheckCircleOutline fontSize="small" /> : <CancelOutlined fontSize="small" />}
              sx={{
                mr: 1,
                p: 1.5,
                fontWeight: 500,
      ...
      <Box sx={{ px: { xxs: 0, xs: 2 }, py: 1 }}>
        ...
        {studentClubsData.detailedDescription && (
          <Box sx={{ mt: 3 }}>
            <Typography
              variant="body2"
              sx={{
            ...
            <Box
              dangerouslySetInnerHTML={{
                __html: DOMPurify.sanitize(studentClubsData?.detailedDescription)
              }}
            />
          </Box>
        )}
        {/* --- Members Section --- */}
        {studentClubsData.members && studentClubsData.members.length > 0 && (
        ...
        {studentClubsData.members.length === 0 && (
