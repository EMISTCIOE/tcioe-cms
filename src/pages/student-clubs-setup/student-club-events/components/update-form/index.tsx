...
import { clearStudentClubEventsData, setEdit } from '../../redux/studentClubEvents.slice';
import CampusKeyOfficialUpdateForm from './Form';

const CampusKeyOfficialEditModal = () => {
  const dispatch = useDispatch();
  const { edit, currentId } = useSelector(studentClubEventsState);

  // Only fetch when we have a valid ID and are in edit mode
  const { data: studentClubEventsData, isLoading } = useRetrieveStudentClubEventsQuery(currentId, { skip: !currentId || !edit });
  ...
  const handleClose = () => {
    ...
    dispatch(clearStudentClubEventsData());
  };
    ...
    <AppDialog
      ...
      content={
        isLoading ? (
          ...
          <CampusKeyOfficialUpdateForm studentClubEventsData={studentClubEventsData} onClose={handleClose} />
        )
      }
    />
  );
